...
app on every push to `main` that touches it (`apps/app` outside `electron/`,
`packages/agent`, or the lockfile), and on demand from the Actions tab — **Run
workflow** takes a profile, so `production` is a click away without editing
anything.
...
```sh
npm run apk:eas:local -w @eunomia/app
```
...
```sh
npm run apk -w @eunomia/app        # or: npm run dist:apk (repo root)
```
...
```sh
npm run android -w @eunomia/app    # build + install a debug build, then Metro
npm start -w @eunomia/app          # Metro alone, against an installed debug build
```
