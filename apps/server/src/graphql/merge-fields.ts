...
const mergeRuleArgs = {
  fromApp: { type: new GraphQLNonNull(GraphQLString) },
  fromContext: { type: GraphQLString },
  toApp: { type: new GraphQLNonNull(GraphQLString) },
  toContext: { type: GraphQLString },
};
...
export function mergeFields(db: Db, entities: Entities) {
  return {
    createMergeRule: {
      ...
      type: new GraphQLNonNull(entities.types.MergeRules!),
      args: mergeRuleArgs,
      resolve: async (_source, args: MergeRuleArgs, ctx: Context) => {
    ...
    applyMergeRules: {
      ...
      // changed. Creating a merge already does this, so it is here for the
      // case creation can't cover: history that arrived afterwards, from a
      // device that was offline or an agent still reporting the old name.
      type: new GraphQLNonNull(GraphQLInt),
      resolve: (_source, _args, ctx: Context) => sweepMergeRules(db, requireUser(ctx)),
    },
  } satisfies Fields;
}
