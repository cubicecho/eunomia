import { sessionFromDeviceKey } from '@eunomia/agent';
import { useEffect, useRef, useState } from 'react';
import { ActivityIndicator, Button, StyleSheet, Text, View } from 'react-native';
import { WebView } from 'react-native-webview';
import type { MobileConfig } from './store.ts';
import { ui } from './ui.tsx';

// The dashboard: the server-hosted web app, signed in with a session minted
// from this device's API key (sessionFromDeviceKey) — same hand-off the
// desktop tray does, so viewing the dashboard never needs a second magic-link
// login. The long-lived API key never enters the page; only the short-lived
